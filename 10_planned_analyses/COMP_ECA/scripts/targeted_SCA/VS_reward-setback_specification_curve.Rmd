...
ggsave(wt, file = "../figures/targeted_SCA/VS_reward_by_motivated.png", height = 6, width  = 8)
```
...
# join panels
(wt = cowplot::plot_grid(top, bottom, ncol = 1, align = "v", labels = c('A', 'B')))
ggsave(wt, file = "../figures/targeted_SCA/VS_reward_by_win_feeling.png", height = 6, width = 8)
...
# join panels
(wt = cowplot::plot_grid(top, bottom, ncol = 1, align = "v", labels = c('A', 'B')))
ggsave(wt, file = "../figures/targeted_SCA/VS_reward_by_motor_speed.png", height = 6, width = 8)
...
## NOTE: THIS IS FEWER SUBJECTS, PC1 effect is only trend.
```{r}
...
## PC1 is NS unless you include motivation
```{r}
VS_reward_mod_PC1 <- lm(beta ~ PC1_log.c + motivated + 
                              #perceived_control +
                          #median_motor_RT_ave + 
                               AGE.c +total_TRs_censored.c,
                             data = FB_df4)
summary(VS_reward_mod_PC1)
```


plot effect
```{r}
effect_df <- data.frame(effect("PC1_log.c", VS_reward_mod_PC1))
...
effect_plot <- ggplot(data = effect_df, aes(x = PC1_log.c, y = fit)) + 
  ...
  geom_jitter(data = FB_df4, aes(x = PC1_log.c, y = beta), 
              width = 0.05, height = 0, alpha = 0.2, color = dark_blue) + 
  ylab("VS Reward Reactivity") + xlab("Cumulative ECA score")

effect_plot
ggsave(effect_plot, file = "../figures/targeted_SCA/VS_reward_by_PC1_effect_plot.png", 
       height = 4, width = 6)
```

```{r}
VS_reward_mod_motiv <- lm(beta ~ PC1_log.c + motivated + 
                               perceived_control + median_motor_RT_ave + 
                               AGE.c + total_TRs_censored.c,
                             data = FB_df4)
summary(VS_reward_mod_motiv)
```

plot effect
```{r}
effect_df <- data.frame(effect("motivated", VS_reward_mod_motiv))
...
effect_plot <- ggplot(data = effect_df, aes(x = motivated, y = fit)) + 
  ...
  geom_jitter(data = FB_df4, aes(x = motivated, y = beta), 
              width = 0.05, height = 0, alpha = 0.2, color = dark_blue) + 
  ylab("VS Reward Reactivity") + xlab("Self-reported Motivation")

effect_plot
ggsave(effect_plot, file = "../figures/targeted_SCA/VS_reward_by_motivation_effect_plot.png", 
       height = 4, width = 6)
```
