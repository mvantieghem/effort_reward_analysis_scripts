...
beh_scan_ECA_long$GROUP_ECA.x <- as.factor(beh_scan_ECA_long$GROUP_ECA.x)
...
ggplot(data = button_RT_df, aes(x = Effort_Choice, y = effort_key_count)) + 
  geom_jitter(color = dark_blue, alpha = 0.1) + 
  geom_point(data = key_press_table, aes(x = Effort_Choice, y = effort_key_count), 
             size = 3, color = "Red") + ylim (0, 16) + 
  geom_errorbar(data = key_press_table, aes(ymin = lower, ymax = upper))
...
# ECA effects on RT during hard/easy tasks:
