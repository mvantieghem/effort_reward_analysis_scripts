---
title: "make covariates for whole brain models: hard reward -setback subset"
...
usable_df <- usable_df %>%
  filter(SUBJECTID %in% hard_FB_contrast_sublist)
nrow(usable_df)
...
write.table(covariates_ECA, file = paste0("covariate_files/COMP_ECA_covariates_hard_FB_contrast_N", N_subs, ".txt"), quote = F, row.names = F, col.names = F)

write.table(covariates_ECA_full, file = paste0("covariate_files/COMP_ECA_covariates_full_mod_hard_FB_contrast_N", N_subs,".txt"), quote = F, row.names = F, col.names = F)
```
...
copes_to_run <- cope_key %>%
  filter(cope_list == "HardReward-HardSetback")
